...
import Foreign.JNI (defineClass, newGlobalRef, deleteLocalRef)
import GHC.Exts (Any)
...
java :: QuasiQuoter
java = QuasiQuoter
    { quoteExp = \txt -> blockOrExpQQ defaultConfig txt
...
-- | Configuration parameters for the java quasiquoter
data Config = Config
    { -- | This hook is called before passing the generated java code to the
      -- 'javac' compiler.
      configGenerationHook :: Java.CompilationUnit -> Java.CompilationUnit
      -- | Instead of placing the generated source and bytecode in a transient
      -- location, use this folder.
    , configOutputDir :: Maybe FilePath
      -- | Embed the generated bytecode in the object file of the module.
      -- If this is @False@, the user needs to make the generated bytecode
      -- avaiable at runtime in some other way.
    , configEmbedOutput :: Bool
    }

-- | Default configuration
defaultConfig :: Config
defaultConfig = Config id Nothing True

-- | A version of the java quasiquoter which takes a configuration
javaWith :: Config -> QuasiQuoter
javaWith config = QuasiQuoter
...
pushWrapperGen
  :: Config -> Q Java.MemberDecl -> Q ()
pushWrapperGen config gen = do
    incrementFinalizerCount
      ...
      isLastFinalizer >>= \case
        True -> do
          ...
          unless (null wrappers) $ do
            embedAsBytecode config "io.tweag.inlinejava" (mangle thismod) $
              configGenerationHook config $ makeCompilationUnit pkgname [] $
...
embedAsBytecode :: Config -> String -> String -> Java.CompilationUnit -> Q ()
embedAsBytecode config pkg name unit = do
  dcs <- TH.runIO $ do
    maybe (withSystemTempDirectory "inlinejava")
          (\d f -> createDirectoryIfMissing True d >> f d)
          (configOutputDir config) $ \dir -> do
...
-- | Holds 'Nothing' if initialization has already happened.
refLoaderClass :: IORef (Maybe (J ('Class "java.lang.ClassLoader")))
refLoaderClass = unsafePerformIO $ do
    loader :: J ('Class "java.lang.ClassLoader") <-
      callStatic "java.lang.ClassLoader" "getSystemClassLoader" []
    (newGlobalRef loader >>= newIORef . Just) <* deleteLocalRef loader
{-# NOINLINE refLoaderClass #-}

-- | Sets the default class loader to use for classes generated by inline-java.
--
-- This should be called before any inline-java quasiquotation is executed.
setDefaultClassLoader :: J ('Class "java.lang.ClassLoader") -> IO ()
setDefaultClassLoader jloader = do
    gloader <- newGlobalRef jloader
    atomicModifyIORef refLoaderClass (\m -> (Just gloader, m)) >>=
      maybe
       (error "setDefaultClassLoader: inline-java has already been initialized")
       (const $ return ())
...
loadJavaWrappers = doit `seq` return ()
  where
    ...
    doit = unsafePerformIO $ do
      keys <- staticPtrKeys
      Just loader <- atomicModifyIORef refLoaderClass $ \m -> (Nothing, m)
      forM_ keys $ \key -> do
